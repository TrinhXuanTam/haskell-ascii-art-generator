module AsciiImageSpec (spec) where
...
spec = do
  describe "AsciiImage" $ do
    it "should convert greyscale to correct character" $ do
      ...
      greyscaleToChar greyscaleValue `shouldBe` greyscaleCharacters !! (125 `div` 25)
  ...
  describe "FilterApplicable AsciiImage" $ do
    prop "InvertFilter should invert colors" $ \asciiImage ->
      ...
      in all (\(orig, inv) -> orig + inv == 255) (zip (concat original) (concat invertedValues))
